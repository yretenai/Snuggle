...
public static class PathFormatter {
    public static string Format(string template, string ext, SerializedObject asset) {
        ...
        template = template.Replace("{Container}", asset.ObjectContainerPath.Replace('{', '\0').SanitizeDirname());
        template = template.Replace("{Ext}", ext.Replace('{', '\0').SanitizeDirname());
        template = template.Replace("{Name}", asset.ToString().Replace('{', '\0').SanitizeDirname());
