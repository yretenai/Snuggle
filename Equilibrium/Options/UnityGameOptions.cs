...
namespace Equilibrium.Options
{
    ...
    public record UnityGameOptions {
        ...
        public bool TryGetOptionsObject<T>(UnityGame game, [MaybeNullWhen(false)] out T options) where T : IUnityGameOptions, new() {
            ...
            options = anonymousOptionsObject.Deserialize<T>(EquilibriumOptions.JsonOptions);
            if (options == null) {
                return false;
            }
        ...
        public void SetOptions(UnityGame game, object options) {
            StorageMap[game] = JsonSerializer.SerializeToElement(options, options.GetType(), EquilibriumOptions.JsonOptions);
        }
