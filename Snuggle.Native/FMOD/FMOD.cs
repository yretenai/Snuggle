...
public struct Memory {
    public static RESULT Initialize(IntPtr poolmem, int poollen, MEMORY_ALLOC_CALLBACK useralloc, MEMORY_REALLOC_CALLBACK userrealloc, MEMORY_FREE_CALLBACK userfree, MEMORY_TYPE memtypeflags = MEMORY_TYPE.ALL) => FMOD5_Memory_Initialize(poolmem, poollen, useralloc, userrealloc, userfree, memtypeflags);

    public static RESULT GetStats(out int currentalloced, out int maxalloced, bool blocking = true) => FMOD5_Memory_GetStats(out currentalloced, out maxalloced, blocking);
...
public struct Thread {
    public static RESULT SetAttributes(THREAD_TYPE type, THREAD_AFFINITY affinity = THREAD_AFFINITY.GROUP_DEFAULT, THREAD_PRIORITY priority = THREAD_PRIORITY.DEFAULT, THREAD_STACK_SIZE stacksize = THREAD_STACK_SIZE.DEFAULT) => FMOD5_Thread_SetAttributes(type, affinity, priority, stacksize);
...
public struct System {
    ...
    public RESULT getDriverInfo(
        ...
        var result = FMOD5_System_GetDriverInfo(
            handle,
            id,
            stringMem,
            namelen,
    ...
    public RESULT setSoftwareChannels(int numsoftwarechannels) => FMOD5_System_SetSoftwareChannels(handle, numsoftwarechannels);
    public RESULT getSoftwareChannels(out int numsoftwarechannels) => FMOD5_System_GetSoftwareChannels(handle, out numsoftwarechannels);

    public RESULT setSoftwareFormat(int samplerate, SPEAKERMODE speakermode, int numrawspeakers) => FMOD5_System_SetSoftwareFormat(handle, samplerate, speakermode, numrawspeakers);

    public RESULT getSoftwareFormat(out int samplerate, out SPEAKERMODE speakermode, out int numrawspeakers) => FMOD5_System_GetSoftwareFormat(handle, out samplerate, out speakermode, out numrawspeakers);

    public RESULT setDSPBufferSize(uint bufferlength, int numbuffers) => FMOD5_System_SetDSPBufferSize(handle, bufferlength, numbuffers);

    public RESULT getDSPBufferSize(out uint bufferlength, out int numbuffers) => FMOD5_System_GetDSPBufferSize(handle, out bufferlength, out numbuffers);
    ...
    public RESULT attachFileSystem(FILE_OPEN_CALLBACK useropen, FILE_CLOSE_CALLBACK userclose, FILE_READ_CALLBACK userread, FILE_SEEK_CALLBACK userseek) => FMOD5_System_AttachFileSystem(handle, useropen, userclose, userread, userseek);
    ...
    public RESULT setCallback(SYSTEM_CALLBACK callback, SYSTEM_CALLBACK_TYPE callbackmask = SYSTEM_CALLBACK_TYPE.ALL) => FMOD5_System_SetCallback(handle, callback, callbackmask);
    ...
    public RESULT getNumNestedPlugins(uint handle, out int count) => FMOD5_System_GetNumNestedPlugins(this.handle, handle, out count);

    public RESULT getNestedPlugin(uint handle, int index, out uint nestedhandle) => FMOD5_System_GetNestedPlugin(this.handle, handle, index, out nestedhandle);

    public RESULT getNumPlugins(PLUGINTYPE plugintype, out int numplugins) => FMOD5_System_GetNumPlugins(handle, plugintype, out numplugins);
    public RESULT getPluginHandle(PLUGINTYPE plugintype, int index, out uint handle) => FMOD5_System_GetPluginHandle(this.handle, plugintype, index, out handle);
    ...
    public RESULT getPluginInfo(uint handle, out PLUGINTYPE plugintype, out uint version) => FMOD5_System_GetPluginInfo(this.handle, handle, out plugintype, IntPtr.Zero, 0, out version);
    ...
    public RESULT createDSPByPlugin(uint handle, out DSP dsp) => FMOD5_System_CreateDSPByPlugin(this.handle, handle, out dsp.handle);

    public RESULT getDSPInfoByPlugin(uint handle, out IntPtr description) => FMOD5_System_GetDSPInfoByPlugin(this.handle, handle, out description);
    ...
    */
    public RESULT registerDSP(ref DSP_DESCRIPTION description, out uint handle) => FMOD5_System_RegisterDSP(this.handle, ref description, out handle);
    /*
    ...
    */
    ...
    // Init/Close.
    public RESULT init(int maxchannels, INITFLAGS flags, IntPtr extradriverdata) => FMOD5_System_Init(handle, maxchannels, flags, extradriverdata);
    ...
    public RESULT setSpeakerPosition(SPEAKER speaker, float x, float y, bool active) => FMOD5_System_SetSpeakerPosition(handle, speaker, x, y, active);

    public RESULT getSpeakerPosition(SPEAKER speaker, out float x, out float y, out bool active) => FMOD5_System_GetSpeakerPosition(handle, speaker, out x, out y, out active);

    public RESULT setStreamBufferSize(uint filebuffersize, TIMEUNIT filebuffersizetype) => FMOD5_System_SetStreamBufferSize(handle, filebuffersize, filebuffersizetype);
    public RESULT getStreamBufferSize(out uint filebuffersize, out TIMEUNIT filebuffersizetype) => FMOD5_System_GetStreamBufferSize(handle, out filebuffersize, out filebuffersizetype);

    public RESULT set3DSettings(float dopplerscale, float distancefactor, float rolloffscale) => FMOD5_System_Set3DSettings(handle, dopplerscale, distancefactor, rolloffscale);
    public RESULT get3DSettings(out float dopplerscale, out float distancefactor, out float rolloffscale) => FMOD5_System_Get3DSettings(handle, out dopplerscale, out distancefactor, out rolloffscale);
    ...
    public RESULT set3DListenerAttributes(int listener, ref VECTOR pos, ref VECTOR vel, ref VECTOR forward, ref VECTOR up) => FMOD5_System_Set3DListenerAttributes(handle, listener, ref pos, ref vel, ref forward, ref up);

    public RESULT get3DListenerAttributes(int listener, out VECTOR pos, out VECTOR vel, out VECTOR forward, out VECTOR up) => FMOD5_System_Get3DListenerAttributes(handle, listener, out pos, out vel, out forward, out up);
    public RESULT set3DRolloffCallback(CB_3D_ROLLOFF_CALLBACK callback) => FMOD5_System_Set3DRolloffCallback(handle, callback);
    public RESULT mixerSuspend() => FMOD5_System_MixerSuspend(handle);

    public RESULT mixerResume() => FMOD5_System_MixerResume(handle);

    public RESULT getDefaultMixMatrix(SPEAKERMODE sourcespeakermode, SPEAKERMODE targetspeakermode, float[] matrix, int matrixhop) => FMOD5_System_GetDefaultMixMatrix(handle, sourcespeakermode, targetspeakermode, matrix, matrixhop);
    public RESULT getSpeakerModeChannels(SPEAKERMODE mode, out int channels) => FMOD5_System_GetSpeakerModeChannels(handle, mode, out channels);

    // System information functions.
    public RESULT getVersion(out uint version) => FMOD5_System_GetVersion(handle, out version);
    public RESULT getOutputHandle(out IntPtr handle) => FMOD5_System_GetOutputHandle(this.handle, out handle);

    public RESULT getChannelsPlaying(out int channels) => FMOD5_System_GetChannelsPlaying(handle, out channels, IntPtr.Zero);

    public RESULT getChannelsPlaying(out int channels, out int realchannels) => FMOD5_System_GetChannelsPlaying(handle, out channels, out realchannels);
    public RESULT getCPUUsage(out CPU_USAGE usage) => FMOD5_System_GetCPUUsage(handle, out usage);

    public RESULT getFileUsage(out long sampleBytesRead, out long streamBytesRead, out long otherBytesRead) => FMOD5_System_GetFileUsage(handle, out sampleBytesRead, out streamBytesRead, out otherBytesRead);
    ...
    public RESULT createSound(byte[] data, MODE mode, ref CREATESOUNDEXINFO exinfo, out Sound sound) => FMOD5_System_CreateSound(handle, data, mode, ref exinfo, out sound.handle);

    public RESULT createSound(IntPtr name_or_data, MODE mode, ref CREATESOUNDEXINFO exinfo, out Sound sound) => FMOD5_System_CreateSound(handle, name_or_data, mode, ref exinfo, out sound.handle);
    ...
    public RESULT createStream(byte[] data, MODE mode, ref CREATESOUNDEXINFO exinfo, out Sound sound) => FMOD5_System_CreateStream(handle, data, mode, ref exinfo, out sound.handle);

    public RESULT createStream(IntPtr name_or_data, MODE mode, ref CREATESOUNDEXINFO exinfo, out Sound sound) => FMOD5_System_CreateStream(handle, name_or_data, mode, ref exinfo, out sound.handle);
    ...
    public RESULT createDSP(ref DSP_DESCRIPTION description, out DSP dsp) => FMOD5_System_CreateDSP(handle, ref description, out dsp.handle);
    public RESULT createDSPByType(DSP_TYPE type, out DSP dsp) => FMOD5_System_CreateDSPByType(handle, type, out dsp.handle);
    ...
    public RESULT playSound(Sound sound, ChannelGroup channelgroup, bool paused, out Channel channel) => FMOD5_System_PlaySound(handle, sound.handle, channelgroup.handle, paused, out channel.handle);

    public RESULT playDSP(DSP dsp, ChannelGroup channelgroup, bool paused, out Channel channel) => FMOD5_System_PlayDSP(handle, dsp.handle, channelgroup.handle, paused, out channel.handle);
    public RESULT getChannel(int channelid, out Channel channel) => FMOD5_System_GetChannel(handle, channelid, out channel.handle);
    public RESULT getDSPInfoByType(DSP_TYPE type, out IntPtr description) => FMOD5_System_GetDSPInfoByType(handle, type, out description);

    public RESULT getMasterChannelGroup(out ChannelGroup channelgroup) => FMOD5_System_GetMasterChannelGroup(handle, out channelgroup.handle);
    public RESULT getMasterSoundGroup(out SoundGroup soundgroup) => FMOD5_System_GetMasterSoundGroup(handle, out soundgroup.handle);

    // Routing to ports.
    public RESULT attachChannelGroupToPort(PORT_TYPE portType, ulong portIndex, ChannelGroup channelgroup, bool passThru = false) => FMOD5_System_AttachChannelGroupToPort(handle, portType, portIndex, channelgroup.handle, passThru);
    public RESULT detachChannelGroupFromPort(ChannelGroup channelgroup) => FMOD5_System_DetachChannelGroupFromPort(handle, channelgroup.handle);

    // Reverb api.
    public RESULT setReverbProperties(int instance, ref REVERB_PROPERTIES prop) => FMOD5_System_SetReverbProperties(handle, instance, ref prop);

    public RESULT getReverbProperties(int instance, out REVERB_PROPERTIES prop) => FMOD5_System_GetReverbProperties(handle, instance, out prop);
    ...
    // Recording api
    public RESULT getRecordNumDrivers(out int numdrivers, out int numconnected) => FMOD5_System_GetRecordNumDrivers(handle, out numdrivers, out numconnected);
    ...
    public RESULT getRecordDriverInfo(
        ...
        var result = FMOD5_System_GetRecordDriverInfo(
            handle,
            id,
            stringMem,
            namelen,
    ...
    public RESULT getRecordPosition(int id, out uint position) => FMOD5_System_GetRecordPosition(handle, id, out position);

    public RESULT recordStart(int id, Sound sound, bool loop) => FMOD5_System_RecordStart(handle, id, sound.handle, loop);
    ...
    // Geometry api
    public RESULT createGeometry(int maxpolygons, int maxvertices, out Geometry geometry) => FMOD5_System_CreateGeometry(handle, maxpolygons, maxvertices, out geometry.handle);
    public RESULT setGeometrySettings(float maxworldsize) => FMOD5_System_SetGeometrySettings(handle, maxworldsize);

    public RESULT getGeometrySettings(out float maxworldsize) => FMOD5_System_GetGeometrySettings(handle, out maxworldsize);

    public RESULT loadGeometry(IntPtr data, int datasize, out Geometry geometry) => FMOD5_System_LoadGeometry(handle, data, datasize, out geometry.handle);

    public RESULT getGeometryOcclusion(ref VECTOR listener, ref VECTOR source, out float direct, out float reverb) => FMOD5_System_GetGeometryOcclusion(handle, ref listener, ref source, out direct, out reverb);
...
public struct Sound {
    ...
    public RESULT unlock(IntPtr ptr1, IntPtr ptr2, uint len1, uint len2) => FMOD5_Sound_Unlock(handle, ptr1, ptr2, len1, len2);
    public RESULT setDefaults(float frequency, int priority) => FMOD5_Sound_SetDefaults(handle, frequency, priority);

    public RESULT getDefaults(out float frequency, out int priority) => FMOD5_Sound_GetDefaults(handle, out frequency, out priority);

    public RESULT set3DMinMaxDistance(float min, float max) => FMOD5_Sound_Set3DMinMaxDistance(handle, min, max);

    public RESULT get3DMinMaxDistance(out float min, out float max) => FMOD5_Sound_Get3DMinMaxDistance(handle, out min, out max);
    public RESULT set3DConeSettings(float insideconeangle, float outsideconeangle, float outsidevolume) => FMOD5_Sound_Set3DConeSettings(handle, insideconeangle, outsideconeangle, outsidevolume);

    public RESULT get3DConeSettings(out float insideconeangle, out float outsideconeangle, out float outsidevolume) => FMOD5_Sound_Get3DConeSettings(handle, out insideconeangle, out outsideconeangle, out outsidevolume);
    public RESULT set3DCustomRolloff(ref VECTOR points, int numpoints) => FMOD5_Sound_Set3DCustomRolloff(handle, ref points, numpoints);

    public RESULT get3DCustomRolloff(out IntPtr points, out int numpoints) => FMOD5_Sound_Get3DCustomRolloff(handle, out points, out numpoints);

    public RESULT getSubSound(int index, out Sound subsound) => FMOD5_Sound_GetSubSound(handle, index, out subsound.handle);

    public RESULT getSubSoundParent(out Sound parentsound) => FMOD5_Sound_GetSubSoundParent(handle, out parentsound.handle);
    ...
    public RESULT getLength(out uint length, TIMEUNIT lengthtype) => FMOD5_Sound_GetLength(handle, out length, lengthtype);

    public RESULT getFormat(out SOUND_TYPE type, out SOUND_FORMAT format, out int channels, out int bits) => FMOD5_Sound_GetFormat(handle, out type, out format, out channels, out bits);
    public RESULT getNumSubSounds(out int numsubsounds) => FMOD5_Sound_GetNumSubSounds(handle, out numsubsounds);

    public RESULT getNumTags(out int numtags, out int numtagsupdated) => FMOD5_Sound_GetNumTags(handle, out numtags, out numtagsupdated);
    ...
    public RESULT getOpenState(out OPENSTATE openstate, out uint percentbuffered, out bool starving, out bool diskbusy) => FMOD5_Sound_GetOpenState(handle, out openstate, out percentbuffered, out starving, out diskbusy);

    public RESULT readData(IntPtr buffer, uint length, out uint read) => FMOD5_Sound_ReadData(handle, buffer, length, out read);
    ...
    public RESULT getSyncPointInfo(IntPtr point, out uint offset, TIMEUNIT offsettype) => FMOD5_Sound_GetSyncPointInfo(handle, point, IntPtr.Zero, 0, out offset, offsettype);
    ...
    public RESULT setLoopPoints(uint loopstart, TIMEUNIT loopstarttype, uint loopend, TIMEUNIT loopendtype) => FMOD5_Sound_SetLoopPoints(handle, loopstart, loopstarttype, loopend, loopendtype);

    public RESULT getLoopPoints(out uint loopstart, TIMEUNIT loopstarttype, out uint loopend, TIMEUNIT loopendtype) => FMOD5_Sound_GetLoopPoints(handle, out loopstart, loopstarttype, out loopend, loopendtype);

    // For MOD/S3M/XM/IT/MID sequenced formats only.
    public RESULT getMusicNumChannels(out int numchannels) => FMOD5_Sound_GetMusicNumChannels(handle, out numchannels);

    public RESULT setMusicChannelVolume(int channel, float volume) => FMOD5_Sound_SetMusicChannelVolume(handle, channel, volume);

    public RESULT getMusicChannelVolume(int channel, out float volume) => FMOD5_Sound_GetMusicChannelVolume(handle, channel, out volume);
    ...
    public Sound(IntPtr ptr) => handle = ptr;
    public bool hasHandle() => handle != IntPtr.Zero;
...
public struct Channel : IChannelControl {
    ...
    public RESULT getPosition(out uint position, TIMEUNIT postype) => FMOD5_Channel_GetPosition(handle, out position, postype);

    public RESULT setChannelGroup(ChannelGroup channelgroup) => FMOD5_Channel_SetChannelGroup(handle, channelgroup.handle);
    public RESULT getChannelGroup(out ChannelGroup channelgroup) => FMOD5_Channel_GetChannelGroup(handle, out channelgroup.handle);
    ...
    public RESULT setLoopPoints(uint loopstart, TIMEUNIT loopstarttype, uint loopend, TIMEUNIT loopendtype) => FMOD5_Channel_SetLoopPoints(handle, loopstart, loopstarttype, loopend, loopendtype);

    public RESULT getLoopPoints(out uint loopstart, TIMEUNIT loopstarttype, out uint loopend, TIMEUNIT loopendtype) => FMOD5_Channel_GetLoopPoints(handle, out loopstart, loopstarttype, out loopend, loopendtype);
    ...
    public RESULT setReverbProperties(int instance, float wet) => FMOD5_Channel_SetReverbProperties(handle, instance, wet);

    public RESULT getReverbProperties(int instance, out float wet) => FMOD5_Channel_GetReverbProperties(handle, instance, out wet);
    ...
    public RESULT setMixLevelsInput(float[] levels, int numlevels) => FMOD5_Channel_SetMixLevelsInput(handle, levels, numlevels);

    public RESULT setMixMatrix(float[] matrix, int outchannels, int inchannels, int inchannel_hop = 0) => FMOD5_Channel_SetMixMatrix(handle, matrix, outchannels, inchannels, inchannel_hop);

    public RESULT getMixMatrix(float[] matrix, out int outchannels, out int inchannels, int inchannel_hop = 0) => FMOD5_Channel_GetMixMatrix(handle, matrix, out outchannels, out inchannels, inchannel_hop);

    // Clock based functionality.
    public RESULT getDSPClock(out ulong dspclock, out ulong parentclock) => FMOD5_Channel_GetDSPClock(handle, out dspclock, out parentclock);

    public RESULT setDelay(ulong dspclock_start, ulong dspclock_end, bool stopchannels = true) => FMOD5_Channel_SetDelay(handle, dspclock_start, dspclock_end, stopchannels);
    public RESULT getDelay(out ulong dspclock_start, out ulong dspclock_end) => FMOD5_Channel_GetDelay(handle, out dspclock_start, out dspclock_end, IntPtr.Zero);

    public RESULT getDelay(out ulong dspclock_start, out ulong dspclock_end, out bool stopchannels) => FMOD5_Channel_GetDelay(handle, out dspclock_start, out dspclock_end, out stopchannels);
    public RESULT addFadePoint(ulong dspclock, float volume) => FMOD5_Channel_AddFadePoint(handle, dspclock, volume);
    public RESULT setFadePointRamp(ulong dspclock, float volume) => FMOD5_Channel_SetFadePointRamp(handle, dspclock, volume);

    public RESULT removeFadePoints(ulong dspclock_start, ulong dspclock_end) => FMOD5_Channel_RemoveFadePoints(handle, dspclock_start, dspclock_end);

    public RESULT getFadePoints(ref uint numpoints, ulong[] point_dspclock, float[] point_volume) => FMOD5_Channel_GetFadePoints(handle, ref numpoints, point_dspclock, point_volume);
    ...
    // 3D functionality.
    public RESULT set3DAttributes(ref VECTOR pos, ref VECTOR vel) => FMOD5_Channel_Set3DAttributes(handle, ref pos, ref vel);
    public RESULT get3DAttributes(out VECTOR pos, out VECTOR vel) => FMOD5_Channel_Get3DAttributes(handle, out pos, out vel);
    public RESULT set3DMinMaxDistance(float mindistance, float maxdistance) => FMOD5_Channel_Set3DMinMaxDistance(handle, mindistance, maxdistance);

    public RESULT get3DMinMaxDistance(out float mindistance, out float maxdistance) => FMOD5_Channel_Get3DMinMaxDistance(handle, out mindistance, out maxdistance);

    public RESULT set3DConeSettings(float insideconeangle, float outsideconeangle, float outsidevolume) => FMOD5_Channel_Set3DConeSettings(handle, insideconeangle, outsideconeangle, outsidevolume);

    public RESULT get3DConeSettings(out float insideconeangle, out float outsideconeangle, out float outsidevolume) => FMOD5_Channel_Get3DConeSettings(handle, out insideconeangle, out outsideconeangle, out outsidevolume);
    public RESULT set3DConeOrientation(ref VECTOR orientation) => FMOD5_Channel_Set3DConeOrientation(handle, ref orientation);

    public RESULT get3DConeOrientation(out VECTOR orientation) => FMOD5_Channel_Get3DConeOrientation(handle, out orientation);

    public RESULT set3DCustomRolloff(ref VECTOR points, int numpoints) => FMOD5_Channel_Set3DCustomRolloff(handle, ref points, numpoints);
    public RESULT get3DCustomRolloff(out IntPtr points, out int numpoints) => FMOD5_Channel_Get3DCustomRolloff(handle, out points, out numpoints);

    public RESULT set3DOcclusion(float directocclusion, float reverbocclusion) => FMOD5_Channel_Set3DOcclusion(handle, directocclusion, reverbocclusion);

    public RESULT get3DOcclusion(out float directocclusion, out float reverbocclusion) => FMOD5_Channel_Get3DOcclusion(handle, out directocclusion, out reverbocclusion);
    ...
    public RESULT set3DDistanceFilter(bool custom, float customLevel, float centerFreq) => FMOD5_Channel_Set3DDistanceFilter(handle, custom, customLevel, centerFreq);
    public RESULT get3DDistanceFilter(out bool custom, out float customLevel, out float centerFreq) => FMOD5_Channel_Get3DDistanceFilter(handle, out custom, out customLevel, out centerFreq);
    ...
    public Channel(IntPtr ptr) => handle = ptr;
    public bool hasHandle() => handle != IntPtr.Zero;
...
public struct ChannelGroup : IChannelControl {
    ...
    public RESULT addGroup(ChannelGroup group, bool propagatedspclock = true) => FMOD5_ChannelGroup_AddGroup(handle, group.handle, propagatedspclock, IntPtr.Zero);

    public RESULT addGroup(ChannelGroup group, bool propagatedspclock, out DSPConnection connection) => FMOD5_ChannelGroup_AddGroup(handle, group.handle, propagatedspclock, out connection.handle);
    public RESULT getNumGroups(out int numgroups) => FMOD5_ChannelGroup_GetNumGroups(handle, out numgroups);

    public RESULT getGroup(int index, out ChannelGroup group) => FMOD5_ChannelGroup_GetGroup(handle, index, out group.handle);
    ...
    public RESULT getChannel(int index, out Channel channel) => FMOD5_ChannelGroup_GetChannel(handle, index, out channel.handle);
    ...
    public RESULT setReverbProperties(int instance, float wet) => FMOD5_ChannelGroup_SetReverbProperties(handle, instance, wet);

    public RESULT getReverbProperties(int instance, out float wet) => FMOD5_ChannelGroup_GetReverbProperties(handle, instance, out wet);
    ...
    public RESULT setMixLevelsInput(float[] levels, int numlevels) => FMOD5_ChannelGroup_SetMixLevelsInput(handle, levels, numlevels);

    public RESULT setMixMatrix(float[] matrix, int outchannels, int inchannels, int inchannel_hop) => FMOD5_ChannelGroup_SetMixMatrix(handle, matrix, outchannels, inchannels, inchannel_hop);
    public RESULT getMixMatrix(float[] matrix, out int outchannels, out int inchannels, int inchannel_hop) => FMOD5_ChannelGroup_GetMixMatrix(handle, matrix, out outchannels, out inchannels, inchannel_hop);

    // Clock based functionality.
    public RESULT getDSPClock(out ulong dspclock, out ulong parentclock) => FMOD5_ChannelGroup_GetDSPClock(handle, out dspclock, out parentclock);

    public RESULT setDelay(ulong dspclock_start, ulong dspclock_end, bool stopchannels) => FMOD5_ChannelGroup_SetDelay(handle, dspclock_start, dspclock_end, stopchannels);
    public RESULT getDelay(out ulong dspclock_start, out ulong dspclock_end) => FMOD5_ChannelGroup_GetDelay(handle, out dspclock_start, out dspclock_end, IntPtr.Zero);

    public RESULT getDelay(out ulong dspclock_start, out ulong dspclock_end, out bool stopchannels) => FMOD5_ChannelGroup_GetDelay(handle, out dspclock_start, out dspclock_end, out stopchannels);
    public RESULT addFadePoint(ulong dspclock, float volume) => FMOD5_ChannelGroup_AddFadePoint(handle, dspclock, volume);

    public RESULT setFadePointRamp(ulong dspclock, float volume) => FMOD5_ChannelGroup_SetFadePointRamp(handle, dspclock, volume);

    public RESULT removeFadePoints(ulong dspclock_start, ulong dspclock_end) => FMOD5_ChannelGroup_RemoveFadePoints(handle, dspclock_start, dspclock_end);

    public RESULT getFadePoints(ref uint numpoints, ulong[] point_dspclock, float[] point_volume) => FMOD5_ChannelGroup_GetFadePoints(handle, ref numpoints, point_dspclock, point_volume);
    ...
    // 3D functionality.
    public RESULT set3DAttributes(ref VECTOR pos, ref VECTOR vel) => FMOD5_ChannelGroup_Set3DAttributes(handle, ref pos, ref vel);

    public RESULT get3DAttributes(out VECTOR pos, out VECTOR vel) => FMOD5_ChannelGroup_Get3DAttributes(handle, out pos, out vel);
    public RESULT set3DMinMaxDistance(float mindistance, float maxdistance) => FMOD5_ChannelGroup_Set3DMinMaxDistance(handle, mindistance, maxdistance);

    public RESULT get3DMinMaxDistance(out float mindistance, out float maxdistance) => FMOD5_ChannelGroup_Get3DMinMaxDistance(handle, out mindistance, out maxdistance);

    public RESULT set3DConeSettings(float insideconeangle, float outsideconeangle, float outsidevolume) => FMOD5_ChannelGroup_Set3DConeSettings(handle, insideconeangle, outsideconeangle, outsidevolume);

    public RESULT get3DConeSettings(out float insideconeangle, out float outsideconeangle, out float outsidevolume) => FMOD5_ChannelGroup_Get3DConeSettings(handle, out insideconeangle, out outsideconeangle, out outsidevolume);
    public RESULT set3DConeOrientation(ref VECTOR orientation) => FMOD5_ChannelGroup_Set3DConeOrientation(handle, ref orientation);
    public RESULT get3DConeOrientation(out VECTOR orientation) => FMOD5_ChannelGroup_Get3DConeOrientation(handle, out orientation);

    public RESULT set3DCustomRolloff(ref VECTOR points, int numpoints) => FMOD5_ChannelGroup_Set3DCustomRolloff(handle, ref points, numpoints);
    public RESULT get3DCustomRolloff(out IntPtr points, out int numpoints) => FMOD5_ChannelGroup_Get3DCustomRolloff(handle, out points, out numpoints);
    public RESULT set3DOcclusion(float directocclusion, float reverbocclusion) => FMOD5_ChannelGroup_Set3DOcclusion(handle, directocclusion, reverbocclusion);

    public RESULT get3DOcclusion(out float directocclusion, out float reverbocclusion) => FMOD5_ChannelGroup_Get3DOcclusion(handle, out directocclusion, out reverbocclusion);
    ...
    public RESULT set3DDistanceFilter(bool custom, float customLevel, float centerFreq) => FMOD5_ChannelGroup_Set3DDistanceFilter(handle, custom, customLevel, centerFreq);
    public RESULT get3DDistanceFilter(out bool custom, out float customLevel, out float centerFreq) => FMOD5_ChannelGroup_Get3DDistanceFilter(handle, out custom, out customLevel, out centerFreq);
    ...
    public ChannelGroup(IntPtr ptr) => handle = ptr;
    public bool hasHandle() => handle != IntPtr.Zero;
...
public struct SoundGroup {
    ...
    public RESULT setMaxAudibleBehavior(SOUNDGROUP_BEHAVIOR behavior) => FMOD5_SoundGroup_SetMaxAudibleBehavior(handle, behavior);
    public RESULT getMaxAudibleBehavior(out SOUNDGROUP_BEHAVIOR behavior) => FMOD5_SoundGroup_GetMaxAudibleBehavior(handle, out behavior);
    ...
    public SoundGroup(IntPtr ptr) => handle = ptr;
    public bool hasHandle() => handle != IntPtr.Zero;
...
public struct DSP {
    ...
    public RESULT addInput(DSP input) => FMOD5_DSP_AddInput(handle, input.handle, IntPtr.Zero, DSPCONNECTION_TYPE.STANDARD);

    public RESULT addInput(DSP input, out DSPConnection connection, DSPCONNECTION_TYPE type = DSPCONNECTION_TYPE.STANDARD) => FMOD5_DSP_AddInput(handle, input.handle, out connection.handle, type);
    public RESULT disconnectFrom(DSP target, DSPConnection connection) => FMOD5_DSP_DisconnectFrom(handle, target.handle, connection.handle);
    ...
    public RESULT getInput(int index, out DSP input, out DSPConnection inputconnection) => FMOD5_DSP_GetInput(handle, index, out input.handle, out inputconnection.handle);

    public RESULT getOutput(int index, out DSP output, out DSPConnection outputconnection) => FMOD5_DSP_GetOutput(handle, index, out output.handle, out outputconnection.handle);
    ...
    public RESULT setWetDryMix(float prewet, float postwet, float dry) => FMOD5_DSP_SetWetDryMix(handle, prewet, postwet, dry);

    public RESULT getWetDryMix(out float prewet, out float postwet, out float dry) => FMOD5_DSP_GetWetDryMix(handle, out prewet, out postwet, out dry);
    public RESULT setChannelFormat(CHANNELMASK channelmask, int numchannels, SPEAKERMODE source_speakermode) => FMOD5_DSP_SetChannelFormat(handle, channelmask, numchannels, source_speakermode);

    public RESULT getChannelFormat(out CHANNELMASK channelmask, out int numchannels, out SPEAKERMODE source_speakermode) => FMOD5_DSP_GetChannelFormat(handle, out channelmask, out numchannels, out source_speakermode);
    ...
    public RESULT setParameterData(int index, byte[] data) => FMOD5_DSP_SetParameterData(handle, index, Marshal.UnsafeAddrOfPinnedArrayElement(data, 0), (uint) data.Length);
    public RESULT getParameterFloat(int index, out float value) => FMOD5_DSP_GetParameterFloat(handle, index, out value, IntPtr.Zero, 0);
    public RESULT getParameterInt(int index, out int value) => FMOD5_DSP_GetParameterInt(handle, index, out value, IntPtr.Zero, 0);

    public RESULT getParameterBool(int index, out bool value) => FMOD5_DSP_GetParameterBool(handle, index, out value, IntPtr.Zero, 0);

    public RESULT getParameterData(int index, out IntPtr data, out uint length) => FMOD5_DSP_GetParameterData(handle, index, out data, out length, IntPtr.Zero, 0);
    ...
    public RESULT getDataParameterIndex(int datatype, out int index) => FMOD5_DSP_GetDataParameterIndex(handle, datatype, out index);
    ...
    public RESULT getInfo(out uint version, out int channels, out int configwidth, out int configheight) => FMOD5_DSP_GetInfo(handle, IntPtr.Zero, out version, out channels, out configwidth, out configheight);
    ...
    // Metering.
    public RESULT setMeteringEnabled(bool inputEnabled, bool outputEnabled) => FMOD5_DSP_SetMeteringEnabled(handle, inputEnabled, outputEnabled);

    public RESULT getMeteringEnabled(out bool inputEnabled, out bool outputEnabled) => FMOD5_DSP_GetMeteringEnabled(handle, out inputEnabled, out outputEnabled);

    public RESULT getMeteringInfo(IntPtr zero, out DSP_METERING_INFO outputInfo) => FMOD5_DSP_GetMeteringInfo(handle, zero, out outputInfo);

    public RESULT getMeteringInfo(out DSP_METERING_INFO inputInfo, IntPtr zero) => FMOD5_DSP_GetMeteringInfo(handle, out inputInfo, zero);

    public RESULT getMeteringInfo(out DSP_METERING_INFO inputInfo, out DSP_METERING_INFO outputInfo) => FMOD5_DSP_GetMeteringInfo(handle, out inputInfo, out outputInfo);

    public RESULT getCPUUsage(out uint exclusive, out uint inclusive) => FMOD5_DSP_GetCPUUsage(handle, out exclusive, out inclusive);
    ...
    public DSP(IntPtr ptr) => handle = ptr;
    public bool hasHandle() => handle != IntPtr.Zero;
...
public struct DSPConnection {
    ...
    public RESULT setMixMatrix(float[] matrix, int outchannels, int inchannels, int inchannel_hop = 0) => FMOD5_DSPConnection_SetMixMatrix(handle, matrix, outchannels, inchannels, inchannel_hop);

    public RESULT getMixMatrix(float[] matrix, out int outchannels, out int inchannels, int inchannel_hop = 0) => FMOD5_DSPConnection_GetMixMatrix(handle, matrix, out outchannels, out inchannels, inchannel_hop);
    ...
    public DSPConnection(IntPtr ptr) => handle = ptr;
    public bool hasHandle() => handle != IntPtr.Zero;
...
public struct Geometry {
    ...
    public RESULT getMaxPolygons(out int maxpolygons, out int maxvertices) => FMOD5_Geometry_GetMaxPolygons(handle, out maxpolygons, out maxvertices);

    public RESULT getPolygonNumVertices(int index, out int numvertices) => FMOD5_Geometry_GetPolygonNumVertices(handle, index, out numvertices);

    public RESULT setPolygonVertex(int index, int vertexindex, ref VECTOR vertex) => FMOD5_Geometry_SetPolygonVertex(handle, index, vertexindex, ref vertex);

    public RESULT getPolygonVertex(int index, int vertexindex, out VECTOR vertex) => FMOD5_Geometry_GetPolygonVertex(handle, index, vertexindex, out vertex);

    public RESULT setPolygonAttributes(int index, float directocclusion, float reverbocclusion, bool doublesided) => FMOD5_Geometry_SetPolygonAttributes(handle, index, directocclusion, reverbocclusion, doublesided);

    public RESULT getPolygonAttributes(int index, out float directocclusion, out float reverbocclusion, out bool doublesided) => FMOD5_Geometry_GetPolygonAttributes(handle, index, out directocclusion, out reverbocclusion, out doublesided);
    ...
    public RESULT setRotation(ref VECTOR forward, ref VECTOR up) => FMOD5_Geometry_SetRotation(handle, ref forward, ref up);

    public RESULT getRotation(out VECTOR forward, out VECTOR up) => FMOD5_Geometry_GetRotation(handle, out forward, out up);
    ...
    public Geometry(IntPtr ptr) => handle = ptr;
    public bool hasHandle() => handle != IntPtr.Zero;
...
public struct Reverb3D {
    ...
    // Reverb manipulation.
    public RESULT set3DAttributes(ref VECTOR position, float mindistance, float maxdistance) => FMOD5_Reverb3D_Set3DAttributes(handle, ref position, mindistance, maxdistance);

    public RESULT get3DAttributes(ref VECTOR position, ref float mindistance, ref float maxdistance) => FMOD5_Reverb3D_Get3DAttributes(handle, ref position, ref mindistance, ref maxdistance);
    public RESULT setProperties(ref REVERB_PROPERTIES properties) => FMOD5_Reverb3D_SetProperties(handle, ref properties);

    public RESULT getProperties(ref REVERB_PROPERTIES properties) => FMOD5_Reverb3D_GetProperties(handle, ref properties);
    ...
    public Reverb3D(IntPtr ptr) => handle = ptr;
    public bool hasHandle() => handle != IntPtr.Zero;
...
public struct StringWrapper {
    ...
    public StringWrapper(IntPtr ptr) => nativeUtf8Ptr = ptr;
...
#pragma warning disable 618
    public static int SizeOf(Type t) => Marshal.SizeOf(t); // Always use Type version, never Object version as it boxes causes GC allocations

    public static object PtrToStructure(IntPtr ptr, Type structureType) => Marshal.PtrToStructure(ptr, structureType);
